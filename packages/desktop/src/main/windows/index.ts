...
import type { DesktopMenuHistoryEntry } from "@opencode/app/desktop-menu"
...
const webContentsListeners = new Set<(contents: WebContents) => void>()
...
type WindowDependencies = {
  readonly fs: FileSystem.FileSystem
  readonly path: Path.Path
  readonly paths: DesktopPaths.Resolved
  readonly storage: DesktopStorage.Interface
  readonly runFork: (effect: Effect.Effect<void>) => void
  readonly wireWindowRecovery: WindowRecovery
...
function trackWebContents(
  ...
  if (rpc) webContentsListeners.forEach((listener) => listener(contents))
}
...
export function subscribeWebContents(listener: (contents: WebContents) => void) {
  webContentsListeners.add(listener)
  return () => webContentsListeners.delete(listener)
}
...
function iconPath(path: Path.Path, paths: DesktopPaths.Resolved) {
  ...
  return path.join(iconsDir(path, paths), `icon.${ext}`)
}
...
export function setPinchZoomEnabled(enabled: boolean) {
  ...
  BrowserWindow.getAllWindows().forEach((win) => {
    ...
    emitIpcEvent(contents, new WindowPinchZoomChanged({ enabled }))
  ...
  const writeLog: WriteLog = (scope, message, annotations = {}, level = "info") => {
    const effect =
      level === "error"
        ...
        : level === "warn"
          ? Effect.logWarning(message, annotations)
          : Effect.logInfo(message, annotations)
    runFork(scoped(scope, effect))
  }
  const dependencies = { fs, path, paths, storage, runFork, wireWindowRecovery, writeLog }
  const create = (id: string = randomUUID()) => createMainWindow(dependencies, id)
  const restore = () => {
    const ids = registry.persisted()
    return (ids.length ? ids : [randomUUID()]).map(create)
  }
...
function createMainWindow(dependencies: WindowDependencies, id: string) {
  ...
  const win = new BrowserWindow({
    ...
    title: "SigmaAgents",
    icon: iconPath(dependencies.path, dependencies.paths),
    ...
    ...(process.platform === "win32"
      ...
      : {}),
    webPreferences: {
      preload: dependencies.paths.preloadPath,
  ...
  const openCodeView = createOpenCodeView(win, id, dependencies)
  const tabbarView = createTabbarView(win, dependencies)
  const tabManager = createDesktopTabManager(win, id, openCodeView, tabbarView, desktopTabs, dependencies)

  allowRendererPermissions(openCodeView.webContents)
  dependencies.wireWindowRecovery(win, openCodeView.webContents, "main", () => relaunchHandler())
  ...
  state.manage(win)
  registerWindow(win, id, dependencies)
  wireFullscreen(win)
  ...
  const reveal = () => {
    ...
    revealed = true
    win.show()
    dependencies.writeLog("window", "main window visible", { window: id })
  }
  const ready = () => {
    contentReady = true
    reveal()
  ...
  if (process.platform === "linux") openCodeView.webContents.once("did-finish-load", ready)
  win.once("closed", () => themeReady.delete(win))
  void loadWebContents(openCodeView.webContents, "index.html").finally(ready)
  return win
}

function wireNavigationPolicy(contents: WebContents, dependencies: WindowDependencies) {
  contents.setWindowOpenHandler(({ url }) => {
    if (!isRendererUrl(url)) dependencies.runFork(openExternalURL(url))
    return { action: "deny" }
  })
  ...
  contents.on("will-navigate", (event, url) => {
    ...
    dependencies.runFork(openExternalURL(url))
  })
}

function registerWindow(win: BrowserWindow, id: string, dependencies: WindowDependencies) {
  ...
  win.on("closed", () => {
    ...
    const files = [windowStateFile(id), desktopTabStateFile(id)]
    dependencies.runFork(
      Effect.gen(function* () {
        yield* Effect.try(() => dependencies.storage.state.clear(windowDataFile(id)))
        yield* Effect.forEach(
          files,
          (file) => dependencies.fs.remove(dependencies.path.join(app.getPath("userData"), file), { force: true }),
          { discard: true },
        )
      }).pipe(
        Effect.tap(() => Effect.sync(() => files.forEach(forgetStore))),
        Effect.catch((error) =>
          scoped("window", Effect.logError("failed to clean window files", { id, error })),
        ),
      ),
    )
  })
...
function createOpenCodeView(win: BrowserWindow, windowID: string, dependencies: WindowDependencies) {
  const view = new WebContentsView({
    webPreferences: {
      ...
      nodeIntegration: false,
      sandbox: true,
      additionalArguments: [windowIDArgument(windowID)],
    },
  })
  primaryWebContents.set(win, view.webContents)
  trackWebContents(win, view.webContents, undefined, true)
...
function createDesktopTabManager(
  ...
  function ensureView(id: DesktopTabID) {
    ...
    if (rendererTab) {
      const view = createRendererTabView(win, windowID, rendererTab, dependencies)
  ...
  function runAction(action: DesktopTabAction) {
    ...
    if (action === "help") {
      dependencies.runFork(openExternalURL(helpURL))
      return
    }
  }
  ...
  const managerValue = {
    load() {
      ...
      void loadWebContents(tabbarWebContents, "tabbar.html").catch((error) => {
        dependencies.writeLog("window", "tabbar load failed", { error }, "error")
      })
      sendState()
    },
    activate,
...
function createRendererTabView(
  ...
  trackWebContents(win, view.webContents, tab, true)
  registerViewContextMenu(view)
  view.webContents.on("did-navigate", notifyDesktopTabHistory)
  view.webContents.on("did-navigate-in-page", notifyDesktopTabHistory)
  allowRendererPermissions(view.webContents)
  dependencies.wireWindowRecovery(win, view.webContents, tab.id, () => relaunchHandler())
  view.setBackgroundColor(backgroundColor ?? defaultBackgroundColor())
  void loadWebContents(view.webContents, tab.html, {
    devUrl: process.env.ELECTRON_7777_RENDERER_URL ?? false,
    devHtml: tab.devHtml,
  }).catch((error) => {
    dependencies.writeLog("window", "renderer tab initial load failed", { tab: tab.id, error }, "error")
  })
  return view
}
...
function createExternalView(
  ...
  view.webContents.setWindowOpenHandler((details) => {
    dependencies.runFork(openExternalURL(details.url))
    return { action: "deny" }
  })
  const handleNavigation = createExternalTabNavigationHandler(tab, (url) =>
    dependencies.runFork(openExternalURL(url)),
  )
  ...
  void restoreExternalTabSession(tab, dependencies)
    .then(() => view.webContents.loadURL(url))
    .catch((error) => {
      dependencies.writeLog("window", "external tab initial load failed", { tab: tab.id, error }, "error")
    })
  return view
}

function restoreExternalTabSession(tab: ExternalDesktopTab, dependencies: WindowDependencies) {
  ...
  const cookies = new Map(
    readExternalTabCookies(tab.partition, dependencies).map((cookie) =>
      [externalTabCookieKey(cookie), cookie] as const,
    ),
  )
  externalSession.cookies.on("changed", (_event, cookie, _cause, removed) => {
    ...
    writeExternalTabCookies(tab.partition, [...cookies.values()], dependencies)
  })
  const restored = Promise.allSettled(
    ...
    .catch((error) => {
      dependencies.writeLog(
        "window",
        "external tab session restore failed",
        { partition: tab.partition, error },
        "error",
      )
    })
...
function readExternalTabCookies(partition: string, dependencies: WindowDependencies) {
  ...
  try {
    ...
    dependencies.writeLog("window", "external tab cookies restore failed", { partition, error }, "error")
    return []
  }
}

function writeExternalTabCookies(
  partition: string,
  cookies: Cookie[],
  dependencies: WindowDependencies,
) {
  if (!safeStorage.isEncryptionAvailable()) {
    dependencies.writeLog("window", "external tab cookies encryption unavailable", { partition }, "warn")
    return
  }
  try {
    ...
    dependencies.writeLog("window", "external tab cookies save failed", { partition, error }, "error")
  }
}
...
function wireFullscreen(win: BrowserWindow) {
  const send = (fullscreen: boolean) => {
    ...
    emitIpcEvent(contents, new WindowFullscreenChanged({ fullscreen }))
  }
...
function updateZoom(win: BrowserWindow) {
  const contents = getPrimaryWebContents(win)
  updateTitlebar(win)
  emitIpcEvent(contents, new WindowZoomChanged({ factor: contents.getZoomFactor() }))
}
