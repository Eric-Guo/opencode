...
export const makeMainWindows = Effect.fn("Window.make")(function* () {
  ...
  const dependencies = { fs, path, paths, runFork, wireWindowRecovery, writeLog }
...
function createMainWindow(dependencies: WindowDependencies, id: string) {
  ...
  const openCodeView = createOpenCodeView(win, dependencies)
...
function registerWindow(win: BrowserWindow, id: string, dependencies: WindowDependencies) {
  ...
  win.on("closed", () => {
    ...
    const files = [windowStateFile(id), windowDataFile(id), desktopTabStateFile(id)]
...
function createOpenCodeView(win: BrowserWindow, dependencies: WindowDependencies) {
...
function createDesktopTabManager(
  ...
  function ensureView(id: DesktopTabID) {
    ...
    if (rendererTab) {
      const view = createRendererTabView(win, rendererTab, dependencies)
...
function createRendererTabView(win: BrowserWindow, tab: RendererDesktopTab, dependencies: WindowDependencies) {
  const view = new WebContentsView({
    webPreferences: {
      preload: dependencies.paths.preloadPath,
      contextIsolation: true,
