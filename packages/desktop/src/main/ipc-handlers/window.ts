  ...
  Effect.gen(function* () {
    ...
    return WindowRpcs.of({
      ...
      WindowGetFocused: (_args, context) =>
        Effect.sync(() => getWindowFromWebContents(sender(handoff, context))?.isFocused() ?? false),
      WindowGetFullscreen: (_args, context) =>
        Effect.sync(() => getWindowFromWebContents(sender(handoff, context))?.isFullScreen() ?? false),
      WindowSetFocus: (_args, context) =>
        Effect.sync(() => getWindowFromWebContents(sender(handoff, context))?.focus()),
      WindowShow: (_args, context) =>
        Effect.sync(() => getWindowFromWebContents(sender(handoff, context))?.show()),
