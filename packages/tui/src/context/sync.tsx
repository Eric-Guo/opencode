    ...
    const result = {
      ...
      session: {
        ...
        addOptimisticPrompt(input: {
          ...
          const match = messages ? search(messages, input.messageID, (m) => m.id) : undefined
          const info: Message = {
            id: input.messageID,
          ...
          batch(() => {
            if (!messages) {
              ...
              setStore(
                ...
                input.sessionID,
                produce((draft) => {
                  draft.splice(match?.index ?? 0, 0, info)
                }),
              )
            }
        ...
        removeOptimisticPrompt(sessionID: string, messageID: string) {
          ...
          const messages = store.message[sessionID]
          const match = messages ? search(messages, messageID, (m) => m.id) : undefined
          batch(() => {
            if (match?.found) {
              setStore(
                "message",
            ...
            setStore(
              produce((draft) => {
                ...
                for (const message of merged.messages) {
                  if (!visibleIDs.has(message.id) && !optimisticMessages.has(message.id)) {
                    delete draft.part[message.id]
                    continue
                  }
                  if (!visibleIDs.has(message.id)) continue
                  const currentParts = draft.part[message.id] ?? []
                  const incoming = merged.parts.get(message.id) ?? currentParts
                  const parts = incoming.flatMap((part) => {
