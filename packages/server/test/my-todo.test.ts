...
it.live("persists the chosen work project through the authenticated HTTP API", () =>
  Effect.gen(function* () {
    ...
    const location = yield* Effect.promise(() =>
      fetch(new URL(`/api/location?location[directory]=${encodeURIComponent(tmp.path)}`, server.base), {
        headers: server.headers,
      }).then((response) => response.json()),
    )
    const url = new URL(`/api/project/${location.project.id}`, server.base)
    const initial = yield* Effect.promise(() =>
      fetch(new URL("/api/project", server.base), { headers: server.headers }),
    )
    expect(initial.status).toBe(200)
    expect(
      (yield* Effect.promise(() => initial.json())).find((project: { id: string }) => project.id === location.project.id)
        .myTodo,
    ).toBeUndefined()
    ...
    expect(
      (yield* Effect.promise(() => restored.json())).find((project: { id: string }) => project.id === location.project.id)
        .myTodo,
    ).toEqual(selected)
