...
const WellKnownRemoteConfig = Schema.Struct({
  url: Schema.String,
...
const WellKnownConfig = Schema.Struct({
  ...
  remote_config: Schema.optional(WellKnownRemoteConfig),
})
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          ...
          const remoteConfigSpec = wellknown.remote_config
          const remote =
            remoteConfigSpec
              ? yield* Effect.gen(function* () {
                  const remoteUrl = yield* substitution
                    .substitute({
                      text: remoteConfigSpec.url,
                      type: "virtual",
                      dir: url,
                      source: wellknownURL,
                      env: authEnv,
                    ...
                    .pipe(Effect.orDie)
                  const remoteHeaders = remoteConfigSpec.headers
                    ? Object.fromEntries(
                        yield* Effect.all(
                          Object.entries(remoteConfigSpec.headers).map(([header, headerValue]) =>
                              substitution
                                .substitute({
                                  text: headerValue,
                                  type: "virtual",
                                ...
                                .pipe(
                                  Effect.orDie,
                                  Effect.map((resolved) => [header, resolved] as const),
                                ),
                            ),
