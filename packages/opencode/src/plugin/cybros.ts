...
function log(effect: Effect.Effect<void>) {
  return runtime.runPromise(effect.pipe(Effect.annotateLogs({ service: "plugin.cybros" })))
}
...
export async function post(trace: Trace, sessionID: string) {
  ...
  if (!token) {
    await log(Effect.logDebug("skipping cybros trace upload; THAPE_SSO_BEARER_API_KEY not set", { sessionID }))
    return
  }
  ...
  const res = await fetch(url, {
    ...
    await log(Effect.logWarning("failed to upload cybros trace", { sessionID, error }))
  })
  if (!res || res.ok) return

  const body = await res.text().catch(() => "")
  await log(
    Effect.logWarning("cybros trace upload returned non-OK status", {
      sessionID,
      status: res.status,
      ...
      return Promise.all([
        ...
        .then(async ([session, msgs]) => {
          ...
          if (app.status === "rejected") {
            await log(Effect.logError("failed to write cybros trace", { sessionID, error: app.reason }))
          }
        })
        .catch(async (err) => {
          await log(Effect.logError("failed to write cybros trace", { sessionID, error: err }))
        })
    },
  }
}
