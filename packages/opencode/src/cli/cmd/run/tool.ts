...
function runCodeSearch(p: ToolProps<CodeSearchTool>): ToolInline {
...
function scrollCodeSearchStart(p: ToolProps<CodeSearchTool>): string {
  const query = text(dict(p.input).query)
  if (!query) {
...
function permCodeSearch(p: ToolPermissionProps<CodeSearchTool>): ToolPermissionInfo {
  const query = text(dict(p.input).query)
  return {
    icon: "◇",
    title: `Exa Code Search "${query}"`,
