...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    const selected = args.provider
      ? options.find(
          (option) =>
            option.key === args.provider ||
            database[option.key]?.name?.toLowerCase() === args.provider?.toLowerCase(),
        )?.value
      : yield* promptValue(
          yield* Prompt.autocomplete({
            message: "Select provider",
    ...
    if (selected === undefined) return yield* fail(`Unknown configured provider "${args.provider}"`)
