...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    const resolved = key ? credentials.find((item) => item.key === key) : undefined
    if (!resolved) return yield* fail(`Unknown configured provider "${args.provider}"`)
    if (resolved.auth === "wellknown") yield* Effect.orDie(authWellKnown.remove(resolved.key))
    else yield* Effect.orDie(authSvc.remove(resolved.key))
