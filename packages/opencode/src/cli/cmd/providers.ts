...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    const options = credentials.map((item, index) => ({
      ...
      providerName: database[item.key]?.name,
      key: item.key,
    }))
    const credential = args.provider
      ? options.find((option) => {
          const providerName = option.providerName?.toLowerCase()
          const query = args.provider?.toLowerCase()
          return option.key === args.provider || (providerName !== undefined && providerName === query)
        })
      : yield* Prompt.select({
          message: "Select provider",
        ...
        }).pipe(Effect.flatMap(promptValue), Effect.map((index) => options[index]))
    if (!credential) return yield* fail(`Unknown configured provider "${args.provider}"`)
    const selectedCredential = credentials[credential.index]
    if (!selectedCredential) return
    if (selectedCredential.auth === "wellknown") yield* Effect.orDie(authWellKnown.remove(selectedCredential.key))
    else yield* Effect.orDie(authSvc.remove(selectedCredential.key))
