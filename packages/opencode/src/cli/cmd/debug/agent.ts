  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      if (toolID) {
        ...
        const result = await executeToolEffect(tool.execute(params, ctx))
...
export function executeToolEffect<A, E>(effect: Effect.Effect<A, E>) {
...
async function getAvailableMcpToolIDs(mcpConfig: Config.Info["mcp"] | undefined) {
  ...
  const tools = await AppRuntime.runPromise(MCP.Service.use((svc) => svc.tools()))
