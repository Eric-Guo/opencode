...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    const read = Effect.fn("CliServerDiscovery.read")(function* () {
      ...
      return Option.getOrUndefined(decodeEntry(JSON.parse(text)))
    })

    const remove = Effect.fn("CliServerDiscovery.remove")(function* () {
      const entry = yield* read()
      if (entry?.pid !== process.pid) return
      yield* Effect.tryPromise(() => rm(file)).pipe(Effect.catchIf(isMissing, () => Effect.void), Effect.orDie)
    })
    ...
    return Service.of({
      write: Effect.fn("CliServerDiscovery.write")(function* (url) {
        ...
        yield* Effect.tryPromise(() =>
          writeFile(file, JSON.stringify({ url: localURL(url).toString(), pid: process.pid }, null, 2), { mode: 0o600 }),
        ).pipe(Effect.orDie)
      }),
      remove,
...
function isMissing(error: unknown): error is { readonly code: "ENOENT" } {
