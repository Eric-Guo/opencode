...
const read = Effect.fn("CliServerDiscovery.read")(function* () {
  const entry = yield* Effect.promise(() => Filesystem.readJson(file)).pipe(Effect.catch(() => Effect.succeed(undefined)))
...
const remove = Effect.fn("CliServerDiscovery.remove")(function* () {
  ...
  yield* Effect.promise(() => unlink(file)).pipe(Effect.ignore)
})
...
export const layer = Layer.succeed(
  ...
  Service.of({
    write: Effect.fn("CliServerDiscovery.write")(function* (url) {
      yield* Effect.promise(() => Filesystem.writeJson(file, { url: localURL(url).toString(), pid: process.pid }, 0o600)).pipe(
        Effect.orDie,
      )
    }),
    remove,
