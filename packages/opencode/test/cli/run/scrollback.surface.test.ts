...
test("renders structured tool finals once without their empty start rows", async () => {
  const cases = [
    {
  ...
  for (const item of cases) {
    ...
    try {
      ...
      try {
        ...
        expect(render(commits)).toContain(item.title)
      } finally {
        destroy(commits)
      }
    } finally {
...
test("renders structured write finals as code blocks", async () => {
  const out = await setup()
