...
const wellKnownAuth = (url: string, input?: { config?: unknown; remoteConfig?: { url: string; headers?: Record<string, string> }; remote?: unknown }) =>
      ...
      Effect.succeed(
        [
          ...
          input?.remote !== undefined
            ? {
                url: input.remoteConfig?.url ?? "https://config.example.com/opencode.json",
                source: input.remoteConfig?.url ?? "https://config.example.com/opencode.json",
                dir: path.dirname(input.remoteConfig?.url ?? "https://config.example.com/opencode.json"),
                content:
...
const wellKnown = (input: {
  ...
  return {
    seen,
    it: configIt({ authWellKnown: wellKnownAuth(input.authUrl ?? "https://example.com", input), client }),
  }
}
