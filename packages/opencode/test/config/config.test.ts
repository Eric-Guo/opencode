...
const substituteEnv = (text: string, env: Record<string, string>) =>
  text.replace(/\{env:([^}]+)\}/g, (_, key) => env[key] ?? process.env[key] ?? "")
...
const wellKnown = (input: {
  ...
  const url = normalizeUrl(input.authUrl ?? "https://example.com")
  ...
  if (input.remoteConfig) {
    const remote = substituteEnv(input.remoteConfig.url, tokenEnv)
    seen.remote = remote
    if (input.remoteConfig.headers?.Authorization) {
      seen.authorization = substituteEnv(input.remoteConfig.headers.Authorization, tokenEnv)
    }
    const remoteContent =
      isRecord(input.remote) && isRecord(input.remote.config) ? input.remote.config : (input.remote ?? {})
    configs.push({
      url: remote,
      source: remote,
  ...
  return {
    seen,
    it: configIt({ authWellKnown: fakeAuthWellKnown }),
  }
}
