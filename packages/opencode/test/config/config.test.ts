...
const layer = Layer.mergeAll(configLayer(), FSUtil.defaultLayer, AppFileSystem.defaultLayer)

const it = testEffect(layer)
const configIt = (options?: Parameters<typeof configLayer>[0]) =>
  testEffect(Layer.mergeAll(configLayer(options), FSUtil.defaultLayer, AppFileSystem.defaultLayer))
...
const wellKnown = (input: {
  ...
  return {
    seen,
    it: configIt({ auth: wellKnownAuth(input.authUrl ?? "https://example.com", input.config ?? input.remote ?? {}), client }),
  }
}
