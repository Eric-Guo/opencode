...
const wellKnown = (input: {
  ...
  const metadata =
    isRecord(input.wellKnown)
      ? input.wellKnown
      : {
          ...(input.config !== undefined ? { config: input.config } : {}),
          ...(input.remoteConfig !== undefined ? { remote_config: input.remoteConfig } : {}),
        }
  const auth = Layer.mock(AuthWellKnown.Service)({
    all: () => Effect.succeed({ [authURL]: new AuthWellKnown.Entry({ key: "TEST_TOKEN", token: "test-token" }) }),
    configs: () =>
      Effect.gen(function* () {
        seen.wellKnown = source
        const docs: AuthWellKnown.ConfigDocument[] = []
        if (isRecord(metadata.config)) {
          docs.push({
            url: authURL,
            source,
        ...
        const remote = metadata.remote_config
        if (!isRecord(remote) || typeof remote.url !== "string") return docs

        const remoteURL = remote.url.replace(/\{env:TEST_TOKEN\}/g, "test-token")
        seen.remote = remoteURL
        const headers = isRecord(remote.headers) ? remote.headers : undefined
        const authorization =
          headers && typeof headers.Authorization === "string"
            ? headers.Authorization.replace(/\{env:TEST_TOKEN\}/g, "test-token")
            : undefined
        if (authorization) seen.authorization = authorization

        const remoteInput = isRecord(input.remote) ? input.remote : undefined
        const remoteContent = remoteInput && isRecord(remoteInput.config) ? remoteInput.config : input.remote
        docs.push({
          url: remoteURL,
          source: remoteURL,
  ...
  return {
    seen,
    it: configIt({ authWellKnown: auth }),
  }
}
