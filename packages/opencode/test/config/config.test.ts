...
const wellKnown = (input: {
  ...
  const metadata =
    isObjectRecord(input.wellKnown)
      ? input.wellKnown
      : {
          ...(input.config !== undefined ? { config: input.config } : {}),
          ...(input.remoteConfig !== undefined ? { remote_config: input.remoteConfig } : {}),
        }
  const substitute = (value: string) => value.replaceAll("{env:TEST_TOKEN}", "test-token")
  const authWellKnown = Layer.mock(AuthWellKnown.Service, {
    configs: () =>
      Effect.gen(function* () {
        ...
        if (isObjectRecord(metadata.config)) {
          documents.push({
            url: authUrl,
            source,
        ...
        if (isObjectRecord(metadata.remote_config) && typeof metadata.remote_config.url === "string") {
          const remoteURL = substitute(metadata.remote_config.url)
          seen.remote = remoteURL
          if (isObjectRecord(metadata.remote_config.headers)) {
            const authHeader = metadata.remote_config.headers["Authorization"]
            if (typeof authHeader === "string") seen.authorization = substitute(authHeader)
          }
          if (input.remoteHtml) {
          ...
          if (!isObjectRecord(input.remote)) return yield* Effect.die(new Error("expected object"))
          documents.push({
            url: remoteURL,
            source: remoteURL,
            dir: path.dirname(remoteURL),
            content: isObjectRecord(input.remote.config) ? input.remote.config : input.remote,
          })
        }
