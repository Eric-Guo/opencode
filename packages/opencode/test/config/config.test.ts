...
const wellKnown = (input: {
  ...
  if (isRecord(metadata) && isRecord(metadata.config)) {
    docs.push({
      url,
      source,
  ...
  if (isRecord(metadata) && isRecord(metadata.remote_config) && typeof metadata.remote_config.url === "string") {
    const remoteURL = metadata.remote_config.url.replace("{env:TEST_TOKEN}", "test-token")
    seen.remote = remoteURL
    if (isRecord(metadata.remote_config.headers) && typeof metadata.remote_config.headers.Authorization === "string") {
      seen.authorization = metadata.remote_config.headers.Authorization.replace("{env:TEST_TOKEN}", "test-token")
    }
    const remote = isRecord(input.remote) && isRecord(input.remote.config) ? input.remote.config : input.remote
    if (remote !== undefined) {
      docs.push({
        url: remoteURL,
        source: remoteURL,
