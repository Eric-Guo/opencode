...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const load: () => Effect.Effect<Record<string, Entry>, Error> = Effect.fnUntraced(function* () {
    ...
    const state = SynchronizedRef.makeUnsafe(
      yield* load().pipe(Effect.orElseSucceed((): Record<string, Entry> => ({}))),
    )
    ...
    const set: Interface["set"] = Effect.fn("AuthWellKnown.set")(function* (url, entry) {
    ...
    const remove: Interface["remove"] = Effect.fn("AuthWellKnown.remove")(function* (url) {
      yield* SynchronizedRef.updateEffect(
        state,
        Effect.fnUntraced(function* (data) {
          const next = { ...data }
