...
describe("Tool", () => {
  ...
  it.effect("executes an unadvertised Code Mode tool only for an explicit debug call", () =>
    Effect.gen(function* () {
      ...
      expect(yield* request.execute(call("echo")).pipe(Effect.flip)).toMatchObject({
        message: 'No tool named "echo" is currently available. Please use a tool from the available tool list.',
      })
