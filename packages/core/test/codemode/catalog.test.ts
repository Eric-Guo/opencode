...
describe("CodeModeInstructions.render", () => {
  test("inlines complete catalogs without search guidance", () => {
    ...
    expect(instructions).toContain("Write a JavaScript orchestration script")
    expect(instructions).toContain("call one tool or combine several")
    expect(instructions).toContain("loops such as `for` and `while` are supported")
    expect(instructions).toContain("Never submit `tools...` expressions as standalone tool names")
  ...
  test("adds search guidance when the catalog exceeds the budget", () => {
    ...
    expect(partial).toContain("Call standalone `tool_search`")
    expect(partial).toContain("inside `execute` code")
    expect(partial).toContain("- tool_search(input: {")
    expect(partial).toContain("Inside that code")
...
describe("CodeModeInstructions.update", () => {
  ...
  test("renders namespace-only deltas without persisting hidden tool entries", () => {
    ...
    expect(text).toContain("call `tool_search` again before relying on previous results")
