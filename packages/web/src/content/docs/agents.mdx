...
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
session_cwd: "{env:OPENCODE_CONFIG_DIR}/knowledge/review"
tools:
  write: false
  edit: false
  bash: false
...
Use `session_cwd` to override the working directory for new ACP sessions started with this agent.
...
This is useful for company/support agents that should always run against a fixed knowledge directory instead of the user-selected project folder.
