...
function renderPromiseClient(groups: ReadonlyArray<Group>) {
  ...
  const fields = groups.map((group) => {
    const methods = group.endpoints.map((endpoint) => {
      ...
      const descriptor = `{ method: ${JSON.stringify(endpoint.endpoint.method)}, path: ${path}${parts.length === 0 ? "" : `, ${parts.join(", ")}`}, successStatuses: [${successStatuses.join(", ")}], declaredStatuses: [${declaredStatuses.join(", ")}], empty: ${endpoint.operation.success === "void"}${isBinarySchema(endpoint.successes[0]) ? ", binary: true" : ""}${binaryBody ? `, binaryBody: true, contentType: ${JSON.stringify(payloadEncoding.contentType)}` : ""} }`
...
function normalizePromiseClientContent(content: string, groups: ReadonlyArray<Group>) {
  ...
  const successReady = replaceOne(
    content,
    "readonly successStatus: number",
    "readonly successStatuses: ReadonlyArray<number>",
  ).replaceAll("response.status !== descriptor.successStatus", "!descriptor.successStatuses.includes(response.status)")
  const sseReady = replaceOne(successReady, "let next: ReadableStreamReadResult<Uint8Array>", "let next")
